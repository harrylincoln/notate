...
import { writeToUserData } from '../utils/user-data';
...
function BarView(props) {
  ...
  const saveActiveNoteToState = ctxCords => {
    if (saveActive) {
      if (Object.keys(savedNotesArr).length === 0) {
        // first
        setSavedNotesArr({
        ...
        setSavedNotesArr({
          ...
          [shadowUserData.activeBarNumber]: [
            ...(savedNotesArr[shadowUserData.activeBarNumber] || []),
            ctxCords,
          ],
        });
      }
  ...
  useEffect(() => {
    ...
    userData.activeBarNumber ? updateBarNumber(userData.activeBarNumber) : updateBarNumber();
  ...
  const handleMouseMove = e => {
    ...
    const closestStave = staves.map.reduce((prev, curr) =>
      Math.abs(offsetY - (curr.y0 - staves.snapTolerance)) <
      Math.abs(offsetY - (prev.y0 - staves.snapTolerance))
        ? curr
        : prev,
    );
    ...
    <>
      <section className="App-content">
        ...
        <div className="toggle-controls-container">
          <button className="btn btn-warning" type="button" onClick={() => clearBar()}>
            Clear bar
          </button>
          <button
            id="reset-app-btn"
